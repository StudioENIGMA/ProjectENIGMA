...
func _ready() -> void:
	...
	passwords_manager_gui.settings_gui_pressed.connect(_on_sub_app_pressed.bindv(["PasswordManager"]))
...
func _on_sub_app_pressed(app_name:String) -> void:
	emit_signal("subscreen_open_requested", app_name)
