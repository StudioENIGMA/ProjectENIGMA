...
func label_text_has_update_format() -> void:
	text_body.text = "O Sistema Operacional atual está na versão " + current_version + ". A versão mais recente é " + new_version + ". Deseja atualizar o sistema?"
...
func none_update_available() -> void:
	text_body.text = "O Sistema Operacional atual está na versão " + current_version + ". " + "Sem atualização disponível."
	update_button.hide()
