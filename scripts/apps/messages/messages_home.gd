...
func _on_storage_answer(
	...
	var conversation_index = conversations_data.find_custom(
		func(conversation:Dictionary):return conversation["name"] == name_parameter
	)
...
func _on_create_message(
	...
	if  conversation_index == -1:
		...
		conversations_data.push_front({
			"name":npc_name,
			"photo":photo_path,
			"messages":[{"message":message, "sender":sender, "time":time, "visualized":false}],
			"options":[]
		})
	else:
		conversations_data[conversation_index]["messages"].append(
			{"message":message, "sender":sender, "time":time, "visualized":false}
		)
